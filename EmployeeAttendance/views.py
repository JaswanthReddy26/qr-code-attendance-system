...
def ViewEmpAttendanceAction(request):
    if request.method == 'POST':
        ...
        output += "</tr>"
        con = MySQLdb.connect(host='127.0.0.1',port = 3306,user = 'root', password = 'root', database = 'emp_attendance',charset='utf8')
        with con:
            cur = con.cursor()
        ...
        with con:
            ...
            for row in rows:
                if row[0] == empid:
                    ...
                    break
        if output == 'none':
            db_connection = MySQLdb.connect(host='127.0.0.1',port = 3306,user = 'root', password = 'root', database = 'emp_attendance',charset='utf8')
